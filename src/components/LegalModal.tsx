...
const content = {
  terms: [
    ...
    { heading: '2. Service Description', body: 'Elijah provides premium private chauffeur and ground transportation services across major cities in Colombia, including but not limited to Bogota, Medellin, Cartagena, Cali, Barranquilla, and Santa Marta.' },
    ...
    { heading: '4. Pricing and Payment', body: 'Prices are calculated based on distance, duration, vehicle type, and time of service. All prices are quoted in Colombian Pesos (COP) unless otherwise stated. Payment is accepted via credit card, debit card, or approved corporate accounts.' },
  ...
  privacy: [
    ...
    { heading: '3. Data Sharing', body: 'We do not sell your personal data to third parties. Information may be shared with our chauffeurs (limited to trip details), payment processors, and as required by Colombian law.' },
    ...
    { heading: '5. Your Rights', body: 'Under Colombian data protection law (Ley 1581 de 2012), you have the right to access, update, rectify, and delete your personal data. Contact us at privacy@elijah.co to exercise these rights.' },
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-[#1E1E1E] rounded-xl shadow-2xl border border-[#2A2520] w-full max-w-lg mx-4 max-h-[85vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#2A2520] flex-shrink-0">
              <div>
                <h2 className="font-serif text-lg text-[#F5F0E8]">{titles[type]}</h2>
                <p className="text-[10px] text-[#9A9590] uppercase tracking-wider mt-0.5">Last updated: March 2026</p>
              </div>
              <button onClick={onClose} className="p-1.5 text-[#9A9590] hover:text-gold transition-colors rounded-lg hover:bg-white/5">
            ...
            <div className="px-6 py-5 overflow-y-auto flex-1 space-y-5">
              {content[type].map((section, i) => (
                <div key={i}>
                  ...
                  <p className="text-sm text-[#9A9590] leading-relaxed">{section.body}</p>
                </div>
              ))}
            </div>

            {/* Footer */}
            <div className="px-6 py-3 border-t border-[#2A2520] flex-shrink-0">
              <button
                onClick={onClose}
                className="w-full py-2 bg-gold hover:bg-gold-hover text-white text-sm font-medium rounded-lg transition-colors"
              >
                I Understand
              </button>
            </div>
